...
var p = [{name: 'John'}, {name: 'Max']
example of test:

var startsWithJ = function(person, i, people)
    returns person.name[0] ==='J';
}

var allJPeople = filter(people, startsWithJ);
...
_.filter = function(collection, test) {
    ...
    _.each(collection, function(value, position, collection) {
        if (test(value, position, collection)) results.push(value);
    });
    return results;
};
...
_.pluck = function(collection, property){
    return _.map(collection, function(e, i, a){
        return e[property];
    })
};
...
_.contains = function(collection, value){
    return _.indexOf(collection, value) === -1 ? false : true ;
};
